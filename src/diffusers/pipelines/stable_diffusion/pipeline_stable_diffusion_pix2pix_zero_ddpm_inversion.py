...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        >>> from diffusers import DDIMScheduler, StableDiffusionPix2PixZeroPipelineDDPM
        ...
        >>> pipeline = StableDiffusionPix2PixZeroPipelineDDPM.from_pretrained(model_ckpt, torch_dtype=torch.float16)
...
EXAMPLE_INVERT_DOC_STRING = """
    Examples:
        ...
        >>> from diffusers import DDIMScheduler, DDIMInverseScheduler, StableDiffusionPix2PixZeroPipelineDDPM
...
class StableDiffusionPix2PixZeroPipelineDDPM(DiffusionPipeline):
    r"""
    ...
    def get_variance(self, timestep):  # , prev_timestep):
        ...
        variance = (beta_prod_t_prev / beta_prod_t) * (
                    1 - alpha_prod_t / alpha_prod_t_prev)  # ((1−αt−1)/(1−αt) ** 0.5) * ((1 - (αt/αt-1)) ** 0.5)
        return variance
    ...
    def __call__(
        ...
        Args:
            ...
            zs (`torch.FloatTensor`, *optional*):
                iid standard normal vectors.
            prompt_embeds (`torch.FloatTensor`, *optional*):
                Pre-generated text embeddings. Can be used to easily tweak text inputs, *e.g.* prompt weighting. If not
                provided, text embeddings will be generated from `prompt` input argument.
            negative_prompt_embeds (`torch.FloatTensor`, *optional*):
        ...
        timesteps = self.scheduler.timesteps if zs is None else self.scheduler.timesteps[-zs.shape[0]:]
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if zs is not None:
                    z = zs[i]
                    latents = self.reverse_step(noise_pred, t, latents, eta=eta, variance_noise=z)
                else:
